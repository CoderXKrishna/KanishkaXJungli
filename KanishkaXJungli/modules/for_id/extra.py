...
from KrishnaX.data import raids, one_word, R7_ban_codes
from KrishnaX import Devs, res_grps, res_devs
from KrishnaX.functions import user_only
...
async def uspam(KanishkaXJungli: Client, e: Message):
    global unlimited
    unlimited = True
    ...
    if re.search(res_devs.lower(), msg.lower()):
       await e.reply("**Sorry !!** I can't Spam On @KrishnaX's owner")
       return

    try:
       while unlimited == True:
           await KanishkaXJungli.send_message(e.chat.id, msg)
    ...
    if LOGS_CHANNEL:
         try:
            await KanishkaXJungli.send_message(LOGS_CHANNEL, f"started Unlimited Spam By User: {e.from_user.id} \n\n Chat: {e.chat.id} \n Spam Message: {msg}")
...
async def uraid(KanishkaXJungli: Client, e: Message):
      global unlimited
      unlimited = True
      ...
      try:
         while unlimited == True:
           ...
           await KanishkaXJungli.send_message(e.chat.id, raid_msg)
...
async def abuse(KanishkaXJungli: Client, e: Message): 
     sex = e.text[7:]
     if sex:
          counts = int(sex)
          if int(e.chat.id) in res_grps:
...
async def echo_(KanishkaXJungli: Client, message: Message):
    ...
    try:
       ...
       await KanishkaXJungli.send_message(message.chat.id, msg)
    except Exception as a:
       await KanishkaXJungli.send_message(message.chat.id, msg)
       print(str(a))

"""Red7 Report through KanishkaXJungli"""
...
async def Red7_report(KanishkaXJungli: Client, message: Message):
   chat = message.chat 
   ...
   if len(txt) == 2:
      try:
         suser = await KanishkaXJungli.get_users(args[0])
      except Exception as a:
         await message.reply_text(f"**ERROR:** {a}")
         return
      if int(suser.id) in Devs:
         await y.reply_text("User is Dev of KanishkaXJungli!")
         return
         ...
         if proof.startswith("https://telegra.ph/file") or proof.startswith("https://telegra.ph"):
            ...
            return
      try:
         await KanishkaXJungli.unblock_user("Red7Systembot")
         Report_message = f"/report {report_user} {report_reason} {report_proof}"
         await KanishkaXJungli.send_message("Red7Systembot", Report_message)
         await asyncio.sleep(2) 
         async for response in KanishkaXJungli.get_chat_history("Red7Systembot", 1):
            hm = await KanishkaXJungli.forward_messages(chat.id, "Red7Systembot", response.id)
