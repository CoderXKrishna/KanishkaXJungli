...
from KanishkaXJungli import AUTO_REACT, EMOJI_LIST
from KanishkaXJungli.core import user_errors 
from KanishkaXJungli.core.help_strings import *
from KrishnaX.functions import user_only, delete_reply
...
def _eval(KanishkaXJungli: Client, message: Message):
...
async def clone_user(KanishkaXJungli: Client, message: Message):
    global FName 
    global LName
    global Bio
    try:
    ...
    if Mai.last_name:
       LName = Mai.last_name
    siu = await KanishkaXJungli.get_chat("me")
    if siu.bio:
       Bio = siu.bio
    Reply = await message.reply("cloning...")
    _bio = await KanishkaXJungli.get_chat(user.id)
    if _bio.bio:
       user_bio = _bio.bio
    else:
       user_bio = None
    pic = await KanishkaXJungli.download_media(user.photo.big_file_id)
    try:
       await KanishkaXJungli.set_profile_photo(photo=pic)
       if user.last_name:
          await KanishkaXJungli.update_profile(first_name=user.first_name, last_name=user.last_name, bio=user_bio)
       else:
          await KanishkaXJungli.update_profile(first_name=user.first_name, bio=user_bio)
...
async def _revert(KanishkaXJungli: Client, message: Message):
    global FName 
    global LName
    global Bio
    Mai = await KanishkaXJungli.get_me()
    photos = [x async for x in KanishkaXJungli.get_chat_photos("me")]
    if not FName:
       await message.reply(f"Error: You didn't cloned anyone!")
       return
    user_bio = Bio
    if not user_bio:
       user_bio = "KanishkaXJungli user!"
    Reply = await message.reply("reverting...")
    try:
       if LName:
          await KanishkaXJungli.update_profile(first_name=FName, last_name=LName, bio=user_bio)
       else:
          await KanishkaXJungli.update_profile(first_name=FName, bio=user_bio)
       await KanishkaXJungli.delete_profile_photos(photos[0].file_id)
...
async def _telegraph(KanishkaXJungli: Client, message: Message):
    ...
    if message.reply_to_message.media:
        ...
        _title = get_text(message) if get_text(message) else KanishkaXJungli.me.first_name
