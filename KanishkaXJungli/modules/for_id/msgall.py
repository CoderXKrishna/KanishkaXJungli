...
async def msgall(KanishkaXJungli: Client, message: Message): 
    ...
    fail = 0
    async for x in KanishkaXJungli.get_chat_members(chat):
       chat_user = x.user
       try:
          await KanishkaXJungli.send_message(chat_user.id, msg)
          done += 1
          await asyncio.sleep(3)
       except Exception as a:
          fail += 1
          print(a)
    await KanishkaXJungli.send_message(user.id, f"Messaged all group members! \n\nsent to `{done}` users \nfailed: `{fail}`")
    await sab.delete()
