...
from KanishkaXJungli import join_errors, leave_errors
...
async def leave(KanishkaXJungli: Client, e: Message):
    ...
    if len(e.text) > 7:
        ...
        try:
           if chat in [-1001321613309, 1321613309, "@DNHxHELL"]:
              return
           await KanishkaXJungli.leave_chat(chat)
        ...
        if LOGS_CHANNEL:
           try:
                await KanishkaXJungli.send_message(LOGS_CHANNEL, f"Leaved Chat \n\n Chat: {chat} \n Cmd By User: {e.from_user.id}")
           except Exception as a:
             print(a)
             pass
