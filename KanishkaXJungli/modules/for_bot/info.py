...
from KrishnaX.functions import get_user, get_id
...
async def _info(KanishkaXJungli: Client, e: Message):
   try:
      user = await get_user(KanishkaXJungli, e)
   ...
   if int(user.id) in Devs:
     msg += "**Rank:** Dev of KanishkaXJungli-"
...
async def _id(KanishkaXJungli: Client, e: Message):
   await get_id(KanishkaXJungli, e)
