...
from KrishnaX.data import spam_usage
from KrishnaX import res_grps, res_devs
from KrishnaX.functions import start_spam, start_dspam, start_pspam
...
async def delayspam(KanishkaXJungli: Client, e: Message): 
    usage = spam_usage.delayspam
    Krishna = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
    Krishnaop = Krishna[1:]
    if len(Krishnaop) == 2:
       counts = int(Krishnaop[0])
       spam_text = str(Krishnaop[1])
       sleeptime = float(Krishna[0])
       await start_dspam(KanishkaXJungli, e, counts, sleeptime, spam_text)
    else:
    ...
    if LOGS_CHANNEL:
         try:
             await KanishkaXJungli.send_message(LOGS_CHANNEL, f"started Delay Spam By User: {e.from_user.id} \n\n Chat: {e.chat.id} \n Counts: {counts} \n Spam Message: {msg} \n Delay Time: {sleeptime}")
...
async def pornspam(KanishkaXJungli: Client, e: Message):
    args = e.text.split(" ", 1)[1].split(" ", 1)
    if args:
       counts = int(args[0])
       await start_pspam(KanishkaXJungli, e, counts)
       
       if LOGS_CHANNEL:
           try:
               await KanishkaXJungli.send_message(LOGS_CHANNEL, f"started Porn Spam By User: {e.from_user.id} \n Chat: {e.chat.id} \n Counts: {count}")
           except Exception as a:
               print(a)
               pass
    else:
...
async def justspam(KanishkaXJungli: Client, e: Message):
    usage = spam_usage.spam
    Krishna = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 1)
    if len(Krishna) == 2:
       counts = int(Krishna[0])
       spam_text = str(Krishna [1])
       await start_spam(KanishkaXJungli, e, counts, spam_text)
    else:
        await e.reply_text(usage.format(handler))
        return 
    if LOGS_CHANNEL:
         try:
            await KanishkaXJungli.send_message(LOGS_CHANNEL, f"started Spam By User: {e.from_user.id} \n\n Chat: {e.chat.id} \n Counts: {counts} \n Spam Message: {msg}")
         except Exception as a:
             print(a)
             pass
...
async def hang(KanishkaXJungli: Client, e: Message): 
    counts = e.text[5:]
    if not counts:
        await e.reply_text("Gime Counts")
        return
    ...
    for _ in range(count):
         await KanishkaXJungli.send_message(e.chat.id, Krishna)
         await asyncio.sleep(0.3)
    if LOGS_CHANNEL:
         try:
            await KanishkaXJungli.send_message(LOGS_CHANNEL, f"started Hang Spam By User: {e.from_user.id} \n\n Chat: {e.chat.id} \n Counts: {count}")
         except Exception as a:
             print(a)
             pass
