...
from KrishnaX.functions import user_reason, user_only, get_user
...
async def ban_user(KanishkaXJungli: Client, message: Message):  
   user, reason = await user_reason(KanishkaXJungli, message, Owner, Sudos)
   try:
      await KanishkaXJungli.ban_chat_member(message.chat.id, user.id)
...
async def unban_user(KanishkaXJungli: Client, message: Message):  
   user = await get_user(KanishkaXJungli, message)
   try:
      await KanishkaXJungli.unban_chat_member(message.chat.id, user.id)
   except ChatAdminRequired:
      await message.reply_text("I'm not admin or I don't have rights.")
      return
   except RightForbidden:
...
async def promote_user(KanishkaXJungli: Client, message: Message):
   chat = message.chat
   user = await get_user(KanishkaXJungli, message)
   try:
     await KanishkaXJungli.promote_chat_member(chat.id, user.id,
...
async def fullpromote(KanishkaXJungli: Client, message: Message):
   chat = message.chat
   user = await get_user(KanishkaXJungli, message)
   try:
     await KanishkaXJungli.promote_chat_member(chat.id, user.id,
...
async def demote_user(KanishkaXJungli: Client, message: Message):
   chat = message.chat
   user = await get_user(KanishkaXJungli, message)
   try:
     await KanishkaXJungli.promote_chat_member(chat.id, user.id,
...
async def pin_message(KanishkaXJungli: Client, message: Message): 
   reply = message.reply_to_message
   if not reply:
     await message.reply_text("Reply to message!")
     return
   try:
     await KanishkaXJungli.pin_chat_message(message.chat.id, reply.id)
...
async def unpin_message(KanishkaXJungli: Client, message: Message):
   reply = message.reply_to_message
   if not reply:
     await message.reply_text("Reply to message!")
     return
   try:
     await KanishkaXJungli.unpin_chat_message(message.chat.id, reply.id)
...
async def _purges(KanishkaXJungli: Client, message: Message):
    ...
    if message.reply_to_message:
        for a_s_message_id in range(
            ...
            if len(message_ids) == 100:
                count_del_etion_s += await KanishkaXJungli.delete_messages(
